...
class NotificationEmailHandlerTest extends munit.FunSuite {
  ...
  test("SalesforcePriceRiseCreateHandler should get records from cohort table and SF") {
    ...
    assertEquals(sentMessages(0).To.ContactAttributes.SubscriberAttributes.AddressLine1, expectedStreet)
    assertEquals(sentMessages(0).To.ContactAttributes.SubscriberAttributes.Town, Some(expectedCity))
    assertEquals(sentMessages(0).To.ContactAttributes.SubscriberAttributes.County, Some(expectedState))
    assertEquals(sentMessages(0).To.ContactAttributes.SubscriberAttributes.Postcode, expectedPostalCode)
    assertEquals(sentMessages(0).To.ContactAttributes.SubscriberAttributes.Country, expectedCountry)
    assertEquals(sentMessages(0).To.ContactAttributes.SubscriberAttributes.FirstName, expectedFirstName)
    assertEquals(sentMessages(0).To.ContactAttributes.SubscriberAttributes.LastName, expectedLastName)
    assertEquals(sentMessages(0).To.ContactAttributes.SubscriberAttributes.NewPrice, expectedNewPrice.toString())
    assertEquals(sentMessages(0).To.ContactAttributes.SubscriberAttributes.StartDate, expectedStartDate.toString())
    assertEquals(sentMessages(0).To.ContactAttributes.SubscriberAttributes.BillingPeriod, expectedBillingPeriod)
  }
}
