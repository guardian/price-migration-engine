...
import pricemigrationengine.model.CohortTableFilter.{AmendmentComplete, MailSendComplete, MailSendProcessingOrError, EstimationComplete, SalesforcePriceRiceCreationComplete}
...
class NotificationMailHandlerTest extends munit.FunSuite {
  ...
  val expectedBillingPeriodInMail = "Monthly"
  ...
  test("NotificationMailHandler should get records from cohort table and SF and send Email with the data") {
    ...
    assertEquals(sentMessages(0).To.ContactAttributes.SubscriberAttributes.payment_frequency, expectedBillingPeriodInMail)
    ...
    assertEquals(
      ...
      CohortItem(
        subscriptionName = expectedSubscriptionName,
        processingStage = MailSendProcessingOrError,
        whenMailSent = Some(StubClock.expectedCurrentTime)
      )
    )
    assertEquals(
  ...
  test("NotificationMailHandler should leave ChortItem in processing state if email send fails") {
    val stubSalesforceClient = stubSFClient(List(salesforceSubscription), List(salesforceContact))
    val updatedResultsWrittenToCohortTable = ArrayBuffer[CohortItem]()
    val stubCohortTable = createStubCohortTable(updatedResultsWrittenToCohortTable, cohortItem)
