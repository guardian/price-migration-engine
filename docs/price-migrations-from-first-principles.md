...
The above explains almost with 100% accuracy the process we use to perform subscription price rises at the Guardian, and for almost the same reasons too. The software that performs that operation is the price migration engine. It's written in Scala + ZIO and presents itself as a state machine running in AWS that performs its daily operations every morning. It essentially talks to Zuora (where the full version of the subscriptions are kept), Salesforce (customer relationship management) and Braze (for email and letter notifications). Although an orchestration machine, the price migration engine is also stateful (maintaining migration states in Dynamo tables).
